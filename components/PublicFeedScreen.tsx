...
import { fetchPublicFeed } from '../lib/api';
import { formatTimeAgo, formatNumber, extractMediaUrls, stripMediaUrls, truncateText } from '../lib/utils';
...
import { extractNostrLinks } from '../lib/utils';
...
// Moved UserProfileHeader to ProfileHeader.tsx
        ...
        try {
            ...
            if (result.success) {
                if (result.profile) {
                    ...
                    if (until === undefined) {
                        fetchProfileFromProfileStr(npub).then(psResult => {
                            if (psResult.success && psResult.profile) {
                                ...
                                setUserProfile((prev: any) => {
                                    ...
                                    return {
                                        ...prev,
                                        // Keep original media/meta if ProfileStr is missing them
                                        ...
                                        // Ensure stats object is also updated for ProfileHeader
                                        stats: {
                                            ...(prev.stats || {}),
                ...
                if (until === undefined) {
                    ...
                    setEvents(prev => {
                        ...
                        // If we didn't get any NEW events, assume no more
                        if (newEvents.length === 0 && result.events.length > 0) {
                            // This might happen if we keep getting the same events
                            setHasMore(false);
                ...
                setError(result.error || 'Failed to fetch public feed');
            }
        } catch (err) {
                ...
                ListHeaderComponent={
                    <View>
                        {onBack && (
                            ...
                            >
                                <Text style={{ fontSize: 20, color: '#f97316', marginRight: 8 }}>←</Text>
                                <Text style={{ color: '#f97316', fontSize: 16, fontWeight: '600' }}>Back</Text>
                            </TouchableOpacity>
                        )}
                        {userProfile ? (
                            <ProfileHeader
...
const styles = StyleSheet.create({
    ...
    listContent: {
        // padding removed from top to allow banner to flush
